...
func (p *Pool) findPool(size int) *sizedPool {
	...
	// 將 size 與最小池大小 (minSize) 進行對比，確定 size 是在多大範圍的池中，對應於最小池大小的倍數
	// 返回除法結果 div 和餘數 rem。
	div, rem := bits.Div64(0, uint64(size), uint64(p.minSize))
	idx := bits.Len64(div)

	// 檢查 div 是否是 2 的幾次方。如果 div 是 2 的冪次方，則會將 idx 減去 1。這樣做是為了處理一些特殊情況，當大小恰好是某個池的大小時，可能希望使用較小的池，而不是使用上一個較大的池。
